...
char* permission_string(struct stat *statbuf) {
    ...
    for(int i = 0; i < 9; i += 1){
        ...
        if(permission_valid) {
            if(perms[i] & S_IRUSR || perms[i] & S_IRGRP || perms[i] & S_IROTH){
                p[i+1] = 'r';
            }
            else if (perms[i] & S_IWUSR || perms[i] & S_IWGRP || perms[i] & S_IXGRP){
                p[i+1] = 'w';
            } 
            else if (perms[i] & S_IROTH || perms[i] & S_IWOTH || perms[i] & S_IXOTH){
                p[i+1] = 'x';
            } 
        }
        else {
